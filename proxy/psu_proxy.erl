...
-define(LISTEN_OPTIONS, [binary, {active, false}, {certfile, "servercert.pem"}, {keyfile, "serverkey.pem"}, {password, "alpha"}]).
-define(CONNECT_OPTIONS, [binary, {active, false}, {certfile, "clientcert.pem"}, {keyfile, "clientkey.pem"}, {password, "alpha"}]).
...
test(CSocket) ->
	case ssl:recv(CSocket, 0, 50) of
		{ok, Packet} ->
			file:write_file("test.bin", Packet);
		{error, closed} ->
...
login_spoof_game(SocketSend, Packet) ->
	...
	SpoofedPacket = <<Before/bits, 192, 168, 1, 15, GamePort:16/little-unsigned-integer, After/bits>>,
...
game_accept(LSocket, GameIP, GamePort) ->
	...
	spawn(fun() -> game_proxy(CSocket, SSocket, "game-client", 1) end),
	spawn(fun() -> game_proxy(SSocket, CSocket, "game-server", 1) end),
	game_accept(LSocket, GameIP, GamePort).
